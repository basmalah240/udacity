...
def intro(current_enemy):
    """Display the game introduction with setting and enemy info."""
...
def cottage_path(has_sword, current_enemy):
    """Handle the outcome of confronting the enemy in their house."""
...
def cave_path(already_visited):
    """Handle the outcome of exploring the cave."""
    if already_visited:
...
def game_loop():
    """Run the main game loop, controlling game state and flow."""
    while True:
        current_enemy = random.choice(enemies_list)
        has_sword = False
        cave_visited = False
        starter_weapon = random.choice(weapons_list)
